...
1.  **Cấu hình Agent:**
    ...
    *   Chọn `USE_LOCAL_AGENT = True` để sử dụng agent cục bộ (tiết kiệm chi phí) hoặc `USE_LOCAL_AGENT = False` để sử dụng agent API miễn phí.
    *   Nếu sử dụng Free API Agent, hãy cập nhật `FREE_API_CONFIG` với API key và endpoint của bạn.
