...
*   **`FREE_API_CONFIG`**: A dictionary containing parameters for the Free API Agent:
    *   `api_key`: Read from `TIENLEN_FREE_API_KEY`.
    *   `endpoint`: Read from `TIENLEN_FREE_API_ENDPOINT`.
...
1.  Set `TIENLEN_USE_LOCAL_AGENT=false` in your environment or change the default in `configs/agent_config.py`.
2.  Set `TIENLEN_FREE_API_KEY` and `TIENLEN_FREE_API_ENDPOINT` in your environment. Optionally set `TIENLEN_FREE_API_MODEL`.
...
1.  Leave `TIENLEN_USE_LOCAL_AGENT` unset, set it to `true`, or keep `USE_LOCAL_AGENT = True` in `configs/agent_config.py`.
